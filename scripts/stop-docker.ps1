<#
.SYNOPSIS
    Stop CloudShirt-Hugo in Docker-modus.

.DESCRIPTION
    Stopt draaiende CloudShirt-Hugo Docker-services via docker compose.
...
try {
    ...
    if (-not (Test-CloudShirtHugoDockerRunning)) {
        Write-Output "Geen draaiende CloudShirt-Hugo Docker-services gevonden."
    }
    else {
        docker compose stop
        Write-Output "CloudShirt-Hugo Docker-services zijn gestopt."
    }
